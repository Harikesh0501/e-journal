...
class UserRepository(BaseRepository):
    ...
    async def store_reset_otp(
        self, user_id: str, otp: str, expires_at: datetime
    ) -> bool:
        """Store password reset OTP code and timestamp."""
        return await self.update_by_id(
            user_id,
            {
                "$set": {
                    ...
                    "reset_otp_sent_at": datetime.now(timezone.utc),
    ...
    async def record_reset_otp_failure(self, user_id: str) -> bool:
        """Increment failed attempts for password reset OTP (SEC-05)."""
        return await self.update_by_id(
            user_id,
            {"$inc": {"reset_otp_failed_attempts": 1}},
        )

    async def reset_password_credentials(
        self, user_id: str, password_hash: str
    ) -> bool:
        """Update password hash, clear reset OTP tokens, and reset failed attempts."""
        return await self.update_by_id(
            user_id,
            {
                "$set": {

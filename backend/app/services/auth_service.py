...
class AuthService:
    ...
    async def register(self, request: UserRegisterRequest, ip_address: str | None = None) -> dict:
        ...
        if request.role != "student":
            raise AppException(
                ...
                message="Public registration is restricted to students. Faculty accounts are provisioned by an administrator.",
    ...
    async def login(self, email: str, password: str, ip_address: str | None = None) -> tuple[str, dict]:
        ...
        if user.get("status") == "suspended":
            raise AppException(
                ...
                message="This account has been suspended by an administrator. Please contact your institution.",
    ...
    async def forgot_password(self, email: str, ip_address: str | None = None) -> dict:
        """Initiate password reset: generate and email 6-digit OTP code."""
        user = await self.user_repo.find_by_email(email)
        if not user:
            raise AppException(
                code=ErrorCode.USER_NOT_FOUND,
        ...
        # 30-second cooldown check (SEC-05)
        now = datetime.now(timezone.utc)
        last_sent = user.get("reset_otp_sent_at")
        if last_sent:
            last_sent_utc = (
            ...
            if (now - last_sent_utc).total_seconds() < 30:
                raise AppException(
                    code=ErrorCode.VALIDATION_ERROR,
                    message="Please wait 30 seconds before requesting another reset code",
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                )
        ...
        await self.user_repo.store_reset_otp(user["id"], otp, otp_expiry)
        ...
        return {"email": user["email"], "message": "Password reset code sent successfully"}

    async def verify_reset_otp(self, email: str, otp: str) -> bool:
        """Validate reset OTP code without updating password."""
        user = await self.user_repo.find_by_email(email)
        if not user:
            raise AppException(
                code=ErrorCode.USER_NOT_FOUND,
        ...
        failed_attempts = user.get("reset_otp_failed_attempts", 0)
        ...
        if not stored_otp or stored_otp != otp:
            await self.user_repo.record_reset_otp_failure(user["id"])
            raise AppException(
                code=ErrorCode.INVALID_OTP,
    ...
    async def reset_password(
        self, email: str, otp: str, new_password: str, ip_address: str | None = None
    ) -> str:
        """Verify password reset OTP and commit new password hash."""
        user = await self.user_repo.find_by_email(email)
        if not user:
            raise AppException(
                code=ErrorCode.USER_NOT_FOUND,
        ...
        await self.user_repo.reset_password_credentials(user["id"], new_password_hash)

...
class JournalService:
    ...
    async def request_changes(
        ...
        try:
            from app.utils.email import send_notification_email
            import html
            student = await self.user_repo.find_by_id(journal["studentId"])
            if student and student.get("email"):
                student_name = student.get("profile", {}).get("name", "Student")
                safe_name = html.escape(str(student_name))
                safe_remarks = html.escape(str(payload.remarks or "Please revise as instructed."))
                safe_title = html.escape(str(assignment.get("title", "Experiment")))
                classroom_name = html.escape(str(classroom.get("name", "Classroom")))
                
                email_subject = f"Changes Requested: Experiment #{exp_num} - {safe_title}"
                email_body = f"""
                <html>
                    <body style="font-family: sans-serif; padding: 20px; color: #171717;">
                        <h2 style="color: #c2410c;">Corrections Requested on Your Journal</h2>
                        <p>Hello <strong>{safe_name}</strong>,</p>
                        <p>Your instructor has reviewed your submission for <strong>Experiment #{exp_num}: {safe_title}</strong> in <strong>{classroom_name}</strong> and requested revisions:</p>
                        <div style="background-color: #fff7ed; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #fdba74;">
                            <p style="margin: 0 0 5px 0;"><strong>Teacher Remarks:</strong></p>
                            <blockquote style="margin: 0; color: #9a3412;">{safe_remarks}</blockquote>
                        </div>
                        <p>Your journal has been unlocked back to draft state. Please open the editor, make the necessary corrections, and re-submit.</p>
                        <br/>
    ...
    async def approve_journal(
        ...
        try:
            ...
            if student and student.get("email"):
                student_name = student.get("profile", {}).get("name", "Student")
                safe_name = html.escape(str(student_name))
                safe_remarks = html.escape(str(payload.remarks or "Great work!")) if payload.remarks else None
                safe_title = html.escape(str(assignment.get("title", "Experiment")))
                classroom_name = html.escape(str(classroom.get("name", "Classroom")))
                
                remarks_html = f"""
                <div style="background-color: #f8fafc; padding: 15px; border-radius: 8px; margin: 15px 0; border: 1px solid #e2e8f0;">
                    <p style="margin: 0 0 5px 0; font-weight: bold; color: #475569;">Teacher Remarks:</p>
                    <p style="margin: 0; color: #1e293b;">{safe_remarks}</p>
                </div>
                """ if safe_remarks else ""

                email_subject = f"Journal Approved: Experiment #{exp_num} - {safe_title}"
                email_body = f"""
                <html>
                    <body style="font-family: sans-serif; padding: 20px; color: #171717;">
                        <h2 style="color: #16a34a;">Experiment Journal Approved!</h2>
                        <p>Hello <strong>{safe_name}</strong>,</p>
                        <p>Your submission for <strong>Experiment #{exp_num}: {safe_title}</strong> in <strong>{classroom_name}</strong> has been evaluated and approved by your instructor.</p>
                        <div style="background-color: #f0fdf4; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #bbf7d0; text-align: center;">
                            <span style="font-size: 14px; color: #15803d; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">Marks Awarded</span>
                            <div style="font-size: 32px; font-weight: 800; color: #166534; margin: 5px 0;">
                                {payload.marks} <span style="font-size: 20px; font-weight: 500; color: #65a30d;">/ {max_marks}</span>
                            </div>
                        </div>
                        {remarks_html}
                        <p>You can view your full evaluated journal and grade in your student dashboard.</p>
                        <br/>
                        <p style="color: #6c757d; font-size: 12px;">This is an automated notification from eJournal. Please do not reply.</p>
                    </body>
                </html>
